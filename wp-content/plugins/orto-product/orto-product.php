...
function gck_get_bundle_offers( $product_id = null ) : array {
    ...
    foreach ( $rows as $row ) {
        ...
        $saving_text   = "Teljes megtakarítás " . number_format($saving_amount, 0, '.', '.') . "Ft";
...
function gck_split_attrs_color_size( array $custom_attrs ) : array {
    ...
    foreach ( $custom_attrs as $key => $attr ) {
        ...
        $is_color = ( strpos($hay, 'oldal') !== false || strpos($hay, 'boja') !== false || strpos($hay, 'barva') !== false || strpos($hay, 'farb') !== false || strpos($hay, 'culoare') !== false || strpos($hay, 'color') !== false || strpos($hay, 'colour') !== false || strpos($hay, 'colore') !== false || strpos($hay, 'kolor') !== false || strpos($hay, 'szín') !== false || strpos($hay, 'szin') !== false || strpos($hay, 'ρώμ') !== false || strpos($hay, 'цвят') !== false );
...
function gck_attr_group_token( string $label, string $type ) : string {
    ...
    if ( $type === 'color' ) {
        $l = str_replace(['szin', 'color', 'colour', ':'], ' ', $l);
    } else {
        $l = str_replace(['meret', 'velicina', 'size', ':'], ' ', $l);
    }
...
function gck_hu_majice_phrase( int $n, bool $free = false, string $type = 'majica' ) : string {
    if ( $type === 'steznica' ) {
        $noun = 'térdrögzítőt';
        return $free ? ( 'ingyen ' . $noun ) : $noun;
    }
    if ( $type === 'bokserica' ) {
        $noun = 'boxeralsót';
    } elseif ( $type === 'carapa' ) {
        $noun = 'zoknit'; // compression socks (accusative of "zokni")
    } else {
        $noun = 'pólót';
    }
    return $free ? ( 'ingyen ' . $noun ) : $noun;
...
function gck_render_bundle_selector() {
    ...
    // Compression socks (HU orto category: orto-kompresziós-zokni).
    // Compression SOCKS only ('zokni' words). Note: the compression MAJICA
    // (orto-kompresijske-majice) must NOT match here — no generic 'kompresz'
    // substring — it keeps the default "majica" noun.
    if (
        has_term( array( 'orto-kompresziós-zokni', 'orto-kompreszios-zokni', 'kompreszios-zokni', 'kompresziós-zokni' ), 'product_cat', $product_id )
        || ( stripos( (string) $product->get_slug(), 'zokni' ) !== false )
        || ( stripos( (string) $product->get_name(), 'zokni' ) !== false )
    ) {
        $gck_garment = 'carapa';
    } elseif (
    ...
    if ( has_term( array( 'orto-cloath' ), 'product_cat', $product_id ) ) {
        $gck_offer_subs = array( 'Fürdőszoba szett', 'Csomag az egész otthonra', 'Csomag családnak és barátoknak' );
    } else {
        $gck_offer_subs = array();
    }
    $gck_offer_badges = has_term( array( 'orto-cloath', 'orto-cloud' ), 'product_cat', $product_id )
        ? array( 1 => 'LEGKELENDŐBB', 2 => 'LEGJOBB ÁR' )
        : array();
    ...
    // to this category. This site's save/sync collapses product attributes to a
    // single Boja/Veličina, so the 3 groups' colour/size options are sourced from
    // hardcoded arrays fed into the EXACT SHBOX swatch/select markup + CSS (see the
    // render branch below). Detected early and bypasses the attribute guards so it
    // always renders regardless of product attributes.
...
.color-fekete   { background: #000; }

.color-feher    { background: #fff; border: 1px solid #ccc; }

.color-szurke   { background: #706d78; }

.color-bez      { background: #e4e0cf; }

.color-sotetkek { background: #2a3262; }
...
.color-kek     { background: #203240; }

.color-piros { background: #ba212f; }
  ...
  margin-right:6px;
    ...
    if (  !has_term( array( 'orto-cloath', 'orto-hyd', 'orto-cloud', 'orto-starter', 'orto-majice', 'orto-polok', 'orto-bokserice', 'orto-boxeralsok', 'orto-polo-boxeralso', 'orto-kompresijske-carape', 'orto-kompresziós-zokni', 'orto-kompreszios-zokni', 'kompreszios-zokni', 'kompresziós-zokni', 'orto-ortopas', 'orto-kneefix', 'orto-controlpro', 'orto-bunion', 'orto-fisiorest', 'orto-norikshers', 'orto-noriks-hers', 'orto-majica-darila', 'orto-leak-boxers', 'orto-kompresijske-majice', 'orto-ortopedski-jastuk', 'orto-kidsnest' ), 'product_cat', $product_id )  )   :
    ?>
        <style>
    ...
    <?php if ( has_term( array( 'orto-kompresziós-zokni', 'orto-kompreszios-zokni', 'kompreszios-zokni', 'kompresziós-zokni' ), 'product_cat', $product_id ) ) : // wider size select for compression socks ?>
        <style>
    ...
    <?php
    // SHGIFTS: the zokni size labels ("39-43" …) are wider than shirt/boxer sizes,
    ...
    <div class="gck-benefits-box">
        ...
        <ul class="gck-benefits-list">
            ...
            <li><strong>✔ Prémium anyagok</strong></li>
            <li><strong>✔ Kényelem kompromisszumok nélkül</strong></li>

            <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
                <li style="color: #c00;"><strong>✔ Kezdőcsomag személyenként csak egyszer vehető igénybe</strong></li>
                <li style="color: #c00;"><strong>✔ 1.000 csomagra limitálva </strong></li>
            <?php endif; ?>
        </ul>
        <?php endif; ?>

        <?php if ( ! $show_countdown && ! $gck_no_attrs && ! $gck_single_size && ! has_term( array( 'orto-kneefix', 'orto-controlpro', 'orto-kompresziós-zokni', 'orto-kompreszios-zokni', 'kompreszios-zokni', 'kompresziós-zokni', 'orto-leak-boxers', 'orto-kompresijske-majice' ), 'product_cat', $product_id ) ) : ?>
    ...
    <div class="gck-top-banner-wrap">
        <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
           ...
           <span>Szerezze be exkluzív <br/> kezdő csomagját</span>
        </div>
    ...
    <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
        <div class="dev-banner" data-total="1000" data-sold="354">
            <div class="dev-banner__text">
                ...
                <span>Korlátozott mennyiség <b class="sold">0</b> / <b class="total">0</b> (Još <b class="remain">0</b> darab)</span>
            </div>
    ...
    <?php if ( $show_countdown ) : ?>
        ...
        <div class="gck-countdown" id="gck-countdown"
            ...
            <div class="gck-countdown__body">
                Az akció csak ma érvényes — még <span class="gck-countdown__timer" aria-live="polite">--:--</span>.<?php
                if ( $countdown_stock > 0 ) :
                    ?> Már csak <strong><?php echo esc_html( $countdown_stock ); ?> db</strong> maradt.<?php
                endif; ?> <strong>Siess, mielőtt elfogy!</strong>
            </div>
        </div>
        <script>
          (function(){
    ...
    <style>
      /* Ortopas: duga imena veličina — puni širine select, manji font, jedan red. */
    ...
    <div id="bundle-selector" class="bundle-box<?php echo $gck_single_size ? ' is-single-size' : ''; ?><?php echo $gck_no_attrs ? ' is-no-attrs' : ''; ?>" data-split-garments="<?php echo $gck_split_garments ? '1' : '0'; ?>">
        ...
        foreach ( $offers as $offer_id => $data ) :
            ...
            // SHGIFTS: real piece count is 6 (4 póló + 1 boxer + 1 zokni), so
            // per-piece pricing reads sensibly, same idea as the SHBOX split.
            ...
            $discount_pct = ( (float) $data['regular'] > 0 )
                ...
                : 0;
        ?>
            <label style="position: relative; <?php if ( ( ( $loop_index == 1 || $loop_index == 3 ) && ! $show_group_titles ) || ( ! empty( $gck_offer_badges ) && isset( $gck_offer_badges[ $loop_index ] ) ) ) : ?> margin-top: 25px;  <?php endif; ?><?php echo $is_default ? ' border-color:#f39c12; background:#f39c1217;' : ''; ?>"
                ...
                <?php if ( ! $show_group_titles ) : ?>
                    <?php if ( $loop_index == 1 ) : ?>
                        <div class="gck-popular-badge">Legnépszerűbb 🔥</div>
                    <?php endif; ?>

                    <?php if ( $loop_index == 3 ) : ?>
                        <div class="gck-popular-badge-2">Legjobb ár 🔥</div>
                    <?php endif; ?>
                <?php endif; ?>
    ...
    && !has_term( array( 'starter-paketi' ), 'product_cat', $product_id ) 
                ...
                <?php if ( $show_price_highlights ) : ?>
                    ...
                    <span class="gck-per-chip">
                        <?php if ( $per_regular > $gck_per_new ) : ?>
                            <span class="gck-per-old"><?php echo number_format( $per_regular, 0, '.', '.' ); ?> Ft</span>
                        <?php endif; ?>
                        <span class="gck-per-new"><?php echo number_format( $gck_per_new, 0, '.', '.' ); ?> Ft / db</span>
                    </span>
                ...
                <?php else : ?>
                    — <span class="bundle-option-title"><?php echo number_format( $gck_per_new, 0, '.', '.' ); ?> Ft / db</span>
                <?php endif; ?>
                ...
                <div class="bundle-total-line">
                    ...
                    <span class="line-total"><?php echo number_format( (float) $data['total'], 0 ); ?>€Ft/span>
                </div>

-->

<div class="bundle-total-line">
    <span style="font-weight:normal;">Teljes:</span>

    <?php if ( ! empty($data['regular']) && (float)$data['regular'] > (float)$data['total'] ) : ?>
        <span class="gck-regular-price">
            <?php echo number_format( (float) $data['regular'], 0, '.', '.' ); ?>Ft
        </span>
    <?php endif; ?>
    ...
    <span class="line-total"><?php echo number_format( (float) $data['total'], 0, '.', '.' ); ?>Ft</span>
                ...
                <div class="bundle-pairs <?php echo $is_default ? '' : 'hidden'; ?>"
                    ...
                    // attributes — "Póló szín"/"Póló méret", "Boxer szín"/"Boxer méret",
                    // "Zokni szín"/"Zokni méret" — grouped by the garment word via the same
                    // helper SHBOX uses ($attr_groups). Form field names stay stable
                    // (majica_color …) so cart/label capture and the socks-width CSS are
                    // unaffected. Falls back to hardcoded options if a garment group can't
                    // be resolved, so it never breaks. Each item gets its own pair index so
                    // all 6 selections reach the cart.
                    $gck_sections = array(
                        array(
                            'label'   => 'Válassz 4 pólót',
                            'gratis'  => false, 'count' => 4, 'gg' => 0, 'garment' => 'majica',
                            'ckey'    => 'majica_color', 'skey' => 'majica_size',
                            'colors'  => array( 'Fekete', 'Fehér', 'Szürke', 'Bézs', 'Sötétkék', 'Barna', 'Zöld' ),
                            'sizes'   => array( 'S', 'M', 'L', 'XL', '2XL', '3XL', '4XL' ),
                        ),
                        array(
                            'label'   => 'Válassz 1 boxert ajándékba',
                            'gratis'  => true, 'count' => 1, 'gg' => 1, 'garment' => 'bokserica',
                            'ckey'    => 'bokserica_color', 'skey' => 'bokserica_size',
                            'colors'  => array( 'Fekete', 'Kék', 'Szürke', 'Zöld', 'Piros' ),
                            'sizes'   => array( 'S', 'M', 'L', 'XL', '2XL', '3XL', '4XL' ),
                        ),
                        array(
                            'label'   => 'Válaszd ki a zokni színét (5 pár)',
                            'gratis'  => true, 'count' => 1, 'gg' => 2, 'garment' => 'carapa',
                            'ckey'    => 'carapa_color', 'skey' => 'carapa_size',
                            'colors'  => array( 'Fekete', 'Fehér' ),
                    ...
                    if ( $gck_split_garments ) {
                        $gck_passes = array(
                            array( 'group' => 0, 'label' => 'Válassz ' . (int) $pairs . ' ' . gck_hu_majice_phrase( $pairs, false, 'majica' ), 'gratis' => false ),
                            array( 'group' => 1, 'label' => 'Válassz még ' . (int) $pairs . ' ' . gck_hu_majice_phrase( $pairs, true, 'bokserica' ), 'gratis' => true ),
                        );
                    } else {
                    ...
                    <?php for ( $i = 1; $i <= $pairs; $i++ ) : ?>
                        <?php if ( $gck_pass['group'] === null && $gck_show_sections && $gck_paid > 0 && $i === 1 ) : ?>
                            <div class="gck-pair-label">Válassz <?php echo (int) $gck_paid; ?> <?php echo esc_html( gck_hu_majice_phrase( $gck_paid, false, $gck_garment ) ); ?></div>
                        <?php endif; ?>
                        <?php if ( $gck_pass['group'] === null && $gck_show_sections && $gck_free > 0 && $i === ( $gck_paid + 1 ) ) : ?>
                            <div class="gck-pair-label is-gratis">Válassz még <?php echo (int) $gck_free; ?> <?php echo esc_html( gck_hu_majice_phrase( $gck_free, true, $gck_garment ) ); ?></div>
                        <?php endif; ?>
                        <div class="bundle-pair">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateBundleUI(selectedRadio) {
        ...
        if (priceElem && total) {
            priceElem.innerHTML = total.replace('.', ',') + ' Ft';
        }
    }
 ...
 * total: póló 1-4 (colour+size), boxer (colour+size), zokni (colour+size). Each
 * line is labelled by inspecting its field keys so the cart / order clearly shows
 * which garment was picked, e.g. "Póló: Fekete - L", "Boxer: Kék - M",
 * "Zokni: Fekete - 39-43". Field-key tokens (majic/bokseric/carap) stay unchanged.
 */
function gck_build_shgifts_lines( array $pairs_data ) : array {
    ...
    foreach ( $pairs_data as $attrs ) {
        ...
        foreach ( array_keys( $attrs ) as $k ) {
            ...
            if ( strpos( $kl, 'majic' ) !== false )    { $label = 'Póló';  break; }
            if ( strpos( $kl, 'bokseric' ) !== false ) { $label = 'Boxer'; break; }
            if ( strpos( $kl, 'carap' ) !== false )    { $label = 'Zokni'; break; }
        }
...
function gck_add_cart_item_data( $cart_item_data, $product_id, $variation_id ) {
    ...
    // SHGIFTS (orto-majica-darila): 6 items (4 póló + 1 boxer + 1 zokni), each
    // under its own pair index, labelled by garment. Strictly gated to that
...
function gck_shgifts_free_gifts_notice() {
    ...
    // Gifts shown: 1x boxer + 5x pár zokni. Images + crossed-out reference prices
    // are editable placeholders — adjust to real photos/prices.
    ...
    $gifts = array(
        array( 'label' => '1x boxer', 'img' => $boxer_img, 'price' => '7.990 Ft' ),
    );
    for ( $i = 0; $i < 5; $i++ ) {
        $gifts[] = array( 'label' => '1x pár zokni', 'img' => $socks_img, 'price' => '4.990 Ft' );
    }
    ?>
    <style>
      .gck-fg{
    ...
    <div class="gck-fg">
        <div class="gck-fg__head">6 INGYENES termék minden vásárláshoz</div>
        <div class="gck-fg__grid">
            <?php foreach ( $gifts as $g ) : ?>
                <div class="gck-fg__card">
                    <span class="gck-fg__badge">Ingyen</span>
