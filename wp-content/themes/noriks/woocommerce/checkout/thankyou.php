 ...
 * Step 2: 6-product grid (after "Nem kérem" or after adding 1 item)
 *
...
foreach ( $order->get_items() as $item ) {
    ...
    $is_majica = ( strpos($cat_str, 'orto-polok') !== false || strpos($cat_str, 'polo') !== false || strpos($name, 'póló') !== false || strpos($cat_str, 'majic') !== false || strpos($name, 'majic') !== false );
    ...
    // Komplet
    $is_komplet = ( strpos($cat_str, 'szett') !== false || strpos($name, 'szett') !== false || strpos($cat_str, 'orto-starter') !== false || strpos($cat_str, 'orto-kezdocsomag') !== false || strpos($cat_str, 'orto-polo-boxeralso') !== false || strpos($name, 'komplet') !== false || strpos($cat_str, 'komplet') !== false );
...
if ( $upsell_is_majice ) {
    ...
    $upsell_qty_prices = array( 1 => 4690, 3 => 10990, 6 => 14590 );
...
<div id="order-splash" style="position:fixed;top:0;left:0;width:100%;height:100%;background:#04ac00;z-index:999999;display:flex;flex-direction:column;align-items:center;justify-content:center;transition:opacity 0.6s ease;">
    ...
    <h1 style="color:#fff;font-family:'Roboto',sans-serif;font-size:28px;font-weight:700;margin:0;">Rendelés beérkezett!</h1>
    <p style="color:rgba(255,255,255,0.85);font-family:'Roboto',sans-serif;font-size:15px;margin:10px 0 0;">Rendelésszám: #<?php echo $order->get_order_number(); ?></p>
</div>
...
<div class="ty-container">
    ...
    <?php if ( $order->has_status( 'failed' ) ) : ?>
        <div class="ty-success" style="background:#fde8e8;">
            ...
            <h1>A rendelés nem sikerült</h1>
            <p>A bank elutasította a tranzakciót. Próbálja újra.</p>
            <p style="margin-top:16px;">
                <a href="<?php echo esc_url( $order->get_checkout_payment_url() ); ?>" style="display:inline-block;background:#E8450E;color:#fff;padding:12px 32px;text-decoration:none;font-weight:700;">Próbálja újra</a>
            </p>
        </div>
    <?php else : ?>
        ...
        <div class="ty-success">
            ...
            <h1>Rendelése beérkezett!</h1>
            <p>A visszaigazolást elküldtük a következő címre: <?php echo esc_html( $order->get_billing_email() ); ?></p>
            <span class="ty-order-num">Rendelés #<?php echo $order->get_order_number(); ?></span>
        </div>

        <!-- ═══ STEP 1: VIGOSHOP UPSELL (COD only) ═══ -->
        <?php if ( $order->get_payment_method() === 'cod' && (float)$order->get_total() <= 48000 ) : ?>
            ...
            <div class="ty_upsell_one_wrapper__popup-content">
                ...
                <div class="tyuo_timer" style="position:relative;">
                    <div class="timer_wrapper">
                        <div class="special_offer_txt">Utolsó esély – az ajánlat lejár</div>
                ...
                <div class="tyuo_middle_section">
                    <div class="sub_title">
                        ...
                        Nincs további szállítási költség – minden egy csomagban
                    </div>
                    <div class="clue_text">
                        ...
                        Egészítse ki a kombinációt és spóroljon
                    </div>
                </div>
                ...
                <div class="tyuo_product_section">
                    ...
                    <div class="ty-qty-picker" style="display:flex;gap:8px;padding:0 0 10px;justify-content:center;">
                        ...
                        <label class="ty-qty-btn<?php echo $cls; ?>" style="flex:1;text-align:center;padding:10px 0;border:2px solid <?php echo $border; ?>;border-radius:4px;font-weight:700;font-size:14px;cursor:pointer;background:<?php echo $bg; ?>;color:#000;">
                            <input type="radio" name="ty_qty" value="<?php echo $q; ?>"<?php echo $chk; ?> style="display:none;"> <?php echo $q; ?>x db
                        </label>
                        <?php endforeach; ?>
                    </div>
                    ...
                    <div class="product_data">
                        ...
                        <div class="right_section_wrapper">
                            ...
                            <div class="product_regular_price" id="ty-upsell-regular"><?php echo number_format($upsell_qty_regular[1], 0, ',', '.'); ?> Ft</div>
                            <div class="product_new_sale_price" id="ty-upsell-price"><?php echo number_format($upsell_qty_prices[1], 0, ',', '.'); ?> Ft</div>
                        </div>
                    </div>

                    <script>
                    (function(){
                        var prices = <?php echo json_encode(array_map(function($p){ return number_format($p,0,',','.') . ' Ft'; }, $upsell_qty_prices)); ?>;
                        var names = <?php echo json_encode($upsell_qty_names); ?>;
                        var images = <?php echo json_encode($upsell_qty_images); ?>;
                        var regulars = <?php echo json_encode(array_map(function($p){ return number_format($p,0,',','.') . ' Ft'; }, $upsell_qty_regular)); ?>;
                        ...
                        <select class="variation-select" id="ty-variation-select">
                            <?php if ( $upsell_variations ) : ?>
                                ...
                                <option value="<?php echo $v['id']; ?>" <?php selected( strtolower($v['size']), strtolower($customer_size) ); ?>>
                                    Fekete, <?php echo esc_html( $v['size'] ); ?>
                                </option>
                                <?php endforeach; ?>
                            <?php else : ?>
                                <option value="">Fekete, S</option>
                                <option value="">Fekete, M</option>
                                <option value="">Fekete, L</option>
                                <option value="">Fekete, XL</option>
                                <option value="">Fekete, XXL</option>
                            <?php endif; ?>
                        </select>
                    </div>
                    ...
                    <div class="buttons-section">
                        <a class="pass-btn" id="ty-btn-skip">Nem kérem</a>
                        <div class="buy-btn" id="ty-btn-add" data-product-id="<?php echo esc_attr( $upsell_product_id ); ?>">HOZZÁADÁS A RENDELÉSHEZ</div>
                    </div>
                </div>
            ...
            <div class="ty-grid-popup">
                <div class="ty-grid-header" style="cursor:pointer;flex-direction:column;position:relative;">
                    ...
                    <div style="display:flex;align-items:center;justify-content:center;gap:10px;margin-bottom:8px;">
                        <div style="font-size:15px;font-weight:400;color:#000;">A különleges ajánlat lejár</div>
                    ...
                    <div style="font-size:20px;font-weight:700;color:#000;line-height:1.3;text-align:center;padding-top:10px;">Adjon hozzá bármely terméket 50% kedvezménnyel</div>
                </div>
                <div class="ty-section-body open" id="ty-grid-body">
                <div class="ty-section-body-inner" style="padding:0;">
                <div class="ty-grid-trust">
                    ✔ Mindent egy csomagban küldünk
                </div>
                <div class="ty-grid">
                    ...
                    <div class="ty-grid-item">
                        ...
                        <div class="g-category">BOXERALSÓ</div>
                        <div class="g-name"><?php echo esc_html( $gp->get_name() ); ?></div>
                        <div class="g-price-old"><?php echo number_format( $gp_price, 0, ',', '.' ); ?> Ft</div>
                        <div class="g-price-new"><?php echo number_format( $gp_sale, 0, ',', '.' ); ?> Ft</div>
                        <?php if ( $gp_vars ) : ?>
                ...
                <div class="buttons-section">
                    <a class="pass-btn" id="ty-grid-close">Nem kérem</a>
                    <div class="buy-btn" id="ty-grid-add-all">HOZZÁADÁS A RENDELÉSHEZ</div>
                </div>
        ...
        <div class="ty-section" id="ty-order-items-section">
            <div class="ty-section-header open" onclick="tyToggle(this)">
                <span id="ty-order-items-header">Rendelés tételei (<?php echo $order->get_item_count(); ?>)</span>
        ...
        <div class="ty-section">
            ...
            <div class="ty-section-body open">
                <div class="ty-section-body-inner">
                    <div class="ty-row"><span class="ty-row-label">Név</span><span class="ty-row-value"><?php echo esc_html( $order->get_billing_first_name() . ' ' . $order->get_billing_last_name() ); ?></span></div>
                    <div class="ty-row"><span class="ty-row-label">Cím</span><span class="ty-row-value"><?php echo esc_html( $order->get_billing_address_1() . ' ' . $order->get_billing_address_2() ); ?></span></div>
                    <div class="ty-row"><span class="ty-row-label">Város</span><span class="ty-row-value"><?php echo esc_html( $order->get_billing_postcode() . ' ' . $order->get_billing_city() ); ?></span></div>
...
<?php else : ?>
    <div class="ty-container">
        <div class="ty-success"><h1>Rendelés</h1>
...
(function(){
    ...
    function tick() {
        if (rem <= 0) {
            ...
            // Auto-open "Rendelés tételei" and "Szállítási cím" sections
        ...
        fetch(ajaxUrl, { method: 'POST', body: rfd })
            ...
            .then(function(d) {
                if (d.success) {
                    ...
                    if (headerSpan) {
                        headerSpan.textContent = 'Rendelés tételei (' + d.data.item_count + ')';
                    }
    ...
    // ─── Step 1: "HOZZÁADÁS" → add to order, then show grid ───
    var addBtn = document.getElementById('ty-btn-add');
    if (addBtn) {
        addBtn.addEventListener('click', function() {
            if (addBtn.disabled) return;
            addBtn.disabled = true;
            addBtn.textContent = 'Hozzáadom...';
            ...
            fetch(ajaxUrl, { method: 'POST', body: fd })
                ...
                .catch(function() {
                    ...
                    addBtn.textContent = 'HOZZÁADÁS A RENDELÉSHEZ';
                });
        });
    }
    ...
    if (overlay) {
        ...
        overlay.querySelectorAll('.g-select-btn').forEach(function(btn) {
            btn.addEventListener('click', function() {
                ...
                if (el.classList.contains('selected')) {
                    ...
                    el.textContent = 'KIVÁLASZTÁS';
                    el.style.background = '#000';
                } else {
                    el.classList.add('selected');
                    el.innerHTML = '✔ KIVÁLASZTVA <span style="font-size:10px;opacity:0.7;margin-left:4px;">eltávolítás</span>';
                    el.style.background = '#2E7D32';
                }
            });
        });

        // "Nem kérem" — close all, show summary without upsells
        document.getElementById('ty-grid-close').addEventListener('click', closeAll);

        // "HOZZÁADÁS A RENDELÉSHEZ" — add all selected items, then close
        ...
        if (gridAddAll) {
            gridAddAll.addEventListener('click', function() {
                ...
                gridAddAll.textContent = 'Hozzáadom...';
                ...
                fetch('<?php echo admin_url("admin-ajax.php"); ?>', { method: 'POST', body: rfd })
                    ...
                    .then(function(d2) {
                        if (d2.success) {
                            ...
                            var headerSpan = document.getElementById('ty-order-items-header');
                            if (headerSpan) {
                                headerSpan.textContent = 'Rendelés tételei (' + d2.data.item_count + ')';
                            }
                        }
                    });
            } else {
