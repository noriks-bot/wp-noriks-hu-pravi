    ...
    jQuery(function($){
      ...
      function validateField(field, force) {
        ...
        if (isEmail && val && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val)) {
          showError($row, '\u2715 Adjon meg érvényes e-mail címet');
          return false;
        }

        /* Phone format (at least 6 digits) */
        if (isPhone && val && val.replace(/\D/g,'').length < 6) {
          showError($row, '\u2715 Adjon meg érvényes telefonszámot');
          return false;
        }
...
add_filter( 'woocommerce_checkout_fields', function( $fields ) {
    ...
    $fields['billing']['billing_first_name']['label'] = 'Keresztnév';
    ...
    $fields['billing']['billing_email']['label'] = 'E-mail cím';
    $fields['billing']['billing_email']['placeholder'] = 'E-mail cím';
    /* Description injected via JS to survive update_checkout AJAX re-renders */
    // $fields['billing']['billing_email']['description'] = 'A rendelés visszaigazolásához és a csomag nyomon követéséhez';
    $fields['billing']['billing_email']['required'] = true;
    $fields['billing']['billing_country']['default'] = 'HU';
...
add_action( 'woocommerce_before_checkout_billing_form', function() {
    echo '<h3 class="checkout-billing-title">Fizetés és Szállítás</h3>';
});

add_filter( 'default_checkout_billing_country', function() { return 'HU'; });
add_filter( 'woocommerce_order_button_text', function() { return 'Megrendelés'; });
...
add_action( 'woocommerce_cart_calculate_fees', function( $cart ) {
    ...
    if ( $chosen_gateway === 'cod' ) {
        $cart->add_fee( 'Utánvét', 1.99, false );
    }
});
...
add_action('woocommerce_review_order_before_submit', function(){
    ...
    <div class="noriks-coupon-wrap" style="margin:12px 0 16px;">
        ...
        <div id="noriks-coupon-expanded" style="display:none;gap:8px;align-items:center;">
            ...
            <button type="button" style="padding:10px 20px;background:#000;color:#fff;border:none;border-radius:6px;font-size:14px;font-weight:600;cursor:pointer;white-space:nowrap;" onclick="noriksApplyCoupon()">Alkalmaz</button>
    ...
    function noriksApplyCoupon(){
        ...
        fetch('<?php echo esc_url(wc_get_checkout_url()); ?>?wc-ajax=apply_coupon',{
            ...
            var isError=!res.ok||res.html.indexOf('error')!==-1||res.html.indexOf('nem található')!==-1||res.html.indexOf('érvénytelen')!==-1||res.html.indexOf('removed')!==-1;
            if(isError){
                msg.style.background='#fde8e8';msg.style.color='#c00';
                var txt=res.html.replace(/<[^>]*>/g,'').trim();
                msg.innerHTML='❌ '+(txt||'A kuponkód érvénytelen.');
            }else{
                msg.style.background='#e8fde8';msg.style.color='#080';
                msg.innerHTML='✅ Kupon alkalmazva!';
                document.getElementById('noriks_coupon_code').value='';
                if(window.jQuery)jQuery('body').trigger('update_checkout');
            }
            btn.textContent='Alkalmaz';btn.disabled=false;
        }).catch(function(){
            msg.style.display='block';msg.style.background='#fde8e8';msg.style.color='#c00';
            msg.textContent='Hiba. Próbálja újra.';btn.textContent='Alkalmaz';btn.disabled=false;
        });
    }
    </script>
    <?php
    endif;
    echo '<h3 class="place-order-title" style="display:block;margin:15px 0 10px;">Rendelés összegzése</h3>';
