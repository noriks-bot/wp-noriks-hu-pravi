	...
	acf_add_local_field_group( array(
		...
		'fields' => array(
			...
			array(
				...
				'label'        => 'Upsell #3 megjelenítése a gomb alatt (NORIKS FIT kompressziós póló)',
				'name'         => 'noriks_pp_upsell3',
				'type'         => 'true_false',
				'instructions' => 'Harmadik upsell doboz: NORIKS FIT kompressziós póló, a vásárló méretet választ. Független az első két kapcsolótól.',
				'ui'           => 1,
			),
		),
		'location'   => array(
...
function noriks_pp_upsell_render() {
	...
	?>
	<div class="npu-wrap">
		<span class="npu-label">Vásárolja együtt és spóroljon:</span>
			...
			<div class="npu-grid">
				...
				<div class="npu-actions">
					<label class="npu-check">
						...
						<span class="npu-check-text">Hozzáadás a vásárláshoz</span>
					</label>

					<select class="npu-size" name="noriks_pp_upsell_size" aria-label="Boxer mérete">
...
function noriks_pp_upsell2_config() {
	return apply_filters( 'noriks_pp_upsell2_config', array(
		...
		'total'      => 6490, // cijena cijelog paketa (2 komada)
		'title'      => '1x Fekete + 1x Szürke póló',
		'desc'       => 'Két alap póló csomagban — adja a rendeléshez %s%% kedvezménnyel.',
		'size_attr'  => 'Méret',
		'sku'        => 'NORIKS-SHIRTS-BLACK-GRAY-2-PACK-UPSELL',
		'image'      => get_template_directory_uri() . '/img/upsell/upsell-2x-majici.png',
		'label'      => 'Vásárolja együtt és spóroljon:',
		'add_text'   => 'Hozzáadás a vásárláshoz',
		'aria'       => 'Pólók mérete',
	) );
}
...
function noriks_pp_upsell2_maybe_add( $cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data ) {
	...
	$lines  = array(
		( $prod_a ? $prod_a->get_name() : 'Fekete' ) . ' - ' . $size,
		( $prod_b ? $prod_b->get_name() : 'Szürke póló' ) . ' - ' . $size,
	);
...
function noriks_pp_upsell3_config() {
	return apply_filters( 'noriks_pp_upsell3_config', array(
		'product_id' => 9421,   // NORIKS FIT kompresijska majica (jednostavan proizvod)
		'total'      => 6990,  // cijena za 1 komad
		'title'      => 'NORIKS FIT kompressziós póló',
		'desc'       => 'Formálja a sziluettet és támogatja a testtartást — add hozzá a rendeléshez %s%% kedvezménnyel.',
		'size_attr'  => 'Méret',
...
function noriks_pp_upsell3_order_item_meta( $item, $cart_item_key, $values, $order ) {
	if ( ! empty( $values['_noriks_pp_upsell3'] ) ) {
		...
		if ( ! empty( $values['_noriks_pp_upsell_size'] ) ) {
			$item->update_meta_data( 'Méret', sanitize_text_field( $values['_noriks_pp_upsell_size'] ) );
		}
	}
}
