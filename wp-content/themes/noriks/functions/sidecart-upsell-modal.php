...
function noriks_ajax_add_to_cart() {
    ...
    if ($cart_item_key) {
        ...
        $msg = !empty($notices) ? strip_tags($notices[0]['notice'] ?? $notices[0]) : 'Hiba a hozzáadáskor';
        wc_clear_notices();
...
function noriks_upsell_modal_markup() {
        ...
        <div class="noriks-modal">
            ...
            <div class="noriks-modal-body">
                ...
                <div class="noriks-modal-qty-row">
                    <span class="noriks-attr-label">MENNYISÉG</span>
                ...
                <div id="noriks-modal-error" class="noriks-modal-error" style="display:none;">Válassza ki az összes opciót</div>
                <button id="noriks-modal-add" class="noriks-modal-add-btn">KOSÁRBA</button>
            </div>
    ...
    (function($) {
        ...
        function renderModal() {
            ...
            $('#noriks-modal-add').text('KOSÁRBA').removeClass('adding added');
        ...
        $(document).on('click', '#noriks-modal-add', function() {
            ...
            if (!variation) {
                $('#noriks-modal-error').text('Ez a kombináció nem elérhető').show();
                return;
            }

            if (!variation.is_in_stock) {
                $('#noriks-modal-error').text('Nincs raktáron').show();
                return;
            }

            $btn.addClass('adding').text('HOZZÁADOM...');
            ...
            $.post(woocommerce_params.ajax_url, data, function(res) {
                ...
                if (res.success !== false && res.fragments) {
                    $btn.removeClass('adding').addClass('added').text('✓ HOZZÁADVA!');
                    ...
                    $btn.removeClass('adding').text('KOSÁRBA');
                    var msg = (res.data && res.data.message) ? res.data.message : 'Hiba a hozzáadáskor';
                ...
                $btn.removeClass('adding').text('KOSÁRBA');
                $('#noriks-modal-error').text('Hiba a hozzáadáskor').show();
            });
        });
