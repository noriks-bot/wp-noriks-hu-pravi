...
function noriks_remove_upsell() {
    ...
    if ( $item->get_meta( '_noriks_upsell' ) !== 'thank you upsell' ) {
        wp_send_json_error( 'Csak upsell termékek távolíthatók el' );
    }

    // Only allow while in primary-hold
    if ( $order->get_status() !== 'primary-hold' ) {
        wp_send_json_error( 'A módosítási idő lejárt' );
    }
    ...
    $order->add_order_note( sprintf( 'Upsell eltávolítva: %s', $product_name ) );

    wp_send_json_success( array( 'message' => 'Eltávolítva' ) );
}
...
function noriks_handle_add_upsell() {
    ...
    $order = wc_get_order( $order_id );
    if ( ! $order ) wp_send_json_error( 'A rendelés nem található' );

    // Only allow upsell on COD orders in primary-hold
    if ( $order->get_payment_method() !== 'cod' ) {
        wp_send_json_error( 'Upsell csak utánvétes rendelésnél elérhető' );
    }
    if ( $order->get_status() !== 'primary-hold' ) {
        wp_send_json_error( 'A hozzáadási idő lejárt' );
    }
    ...
    if ( ! $product ) wp_send_json_error( 'A termék nem található' );
    ...
    foreach ( $order->get_items() as $item ) {
        ...
        if ( $item_product_id == $check_product_id || ( $variation_id && $item_variation_id == $variation_id ) ) {
            if ( $item->get_meta( '_noriks_upsell' ) ) {
                wp_send_json_error( 'Már hozzáadta ezt a terméket' );
            }
        }
    }
    ...
    if ( ! $active_price ) {
        wp_send_json_error( 'A termék ára nem elérhető' );
    }

    $quantity = max( 1, absint( $_POST['quantity'] ?? 3 ) );
    // Prices depend on product type (bokserice vs majice)
    $bokserice_prices = array( 1 => 1790, 3 => 5370, 5 => 8950 );
    $majice_prices    = array( 1 => 4690, 3 => 10990, 6 => 14590 );
    ...
    $is_majice = ( strpos($cat_str, 'orto-polok') !== false || strpos($cat_str, 'polo') !== false || strpos($name, 'póló') !== false || strpos($cat_str, 'majic') !== false || strpos($name, 'majic') !== false );
    ...
    if ( ! $item_id ) wp_send_json_error( 'Hiba a hozzáadás során' );
    ...
    $order->add_order_note(
        sprintf(
            'Thank you upsell: %s hozzáadva 50%% kedvezménnyel — akciós ár: %s, upsell ár: %s',
            $product->get_name(),
