...
function noriks_remove_upsell() {
    ...
    if ( $item->get_meta( '_noriks_upsell' ) !== 'thank you upsell' ) {
        wp_send_json_error( 'Csak az upsell termékek távolíthatók el' );
    }
...
function noriks_handle_add_upsell() {
    ...
    if ( $order->get_payment_method() !== 'cod' ) {
        wp_send_json_error( 'Upsell csak utánvétes fizetésnél érhető el' );
    }
    if ( $order->get_status() !== 'primary-hold' ) {
    ...
    foreach ( $order->get_items() as $item ) {
        ...
        if ( $item_product_id == $check_product_id || ( $variation_id && $item_variation_id == $variation_id ) ) {
            if ( $item->get_meta( '_noriks_upsell' ) ) {
                wp_send_json_error( 'Ezt a terméket már hozzáadta' );
            }
        }
    }
    ...
    $bokserice_prices = array( 1 => 3190, 3 => 7990, 5 => 11990 );
    $majice_prices    = array( 1 => 5190, 3 => 11990, 6 => 15990 );
    $name = strtolower($product->get_name());
    $is_majice = strpos($name, 'póló') !== false || strpos($name, 'polo') !== false || strpos($name, 'majic') !== false;
    ...
    if ( ! $item_id ) wp_send_json_error( 'Hiba a hozzáadáskor' );
