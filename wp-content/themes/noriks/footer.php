...
<!-- suport section -->
	...
	if ( ! $nf_legal ) {
		$nf_legal = 'NORIKS BRAND. Minden jog fenntartva. Az árak tartalmazzák az áfát. A képek illusztrációk.';
	}
	?>
		...
		<section class="nf-signup">
			<div class="nf-wrap nf-signup-grid">
				<div class="nf-signup-copy">
					<h2 class="nf-signup-title">Csatlakozz a NORIKS családhoz</h2>
					<p class="nf-signup-sub">Exkluzív ajánlatok és új termékek — értesülj elsőként.</p>
				</div>
				<div class="nf-signup-form-col">
					<form class="nf-signup-form" data-klaviyo-list="<?php echo esc_attr( $nf_klaviyo_list ); ?>" novalidate>
						<label class="nf-field-label" for="nf-email">Az e-mail címed *</label>
						<div class="nf-field-row">
							<input type="email" id="nf-email" name="email" class="nf-input" placeholder="E-mail címed" required autocomplete="email">
							<button type="submit" class="nf-btn nf-btn-light">Feliratkozás</button>
						</div>
		...
		<section class="nf-main">
			<div class="nf-wrap nf-main-grid">
				...
				<div class="nf-brand-col">
					<p class="nf-brand-tagline">Ruházat az erősebb testalkatra, úgy szabva, hogy valóban jól álljon.</p>
					<p class="nf-brand-desc"><?php echo esc_html( $nf_brand ); ?></p>
					<a class="nf-btn nf-btn-outline" href="/hu/shop">Nézd meg a kollekciót</a>
				</div>
						...
						<div class="nf-link-col">
							...
							if ( $nf_col2_l ) :
								foreach ( $nf_col2_l as $item ) :
									if ( false !== strpos( (string) $item['link'], '/utmutatok' ) ) { $nf_has_upute = true; }
									?>
								<a href="<?php echo esc_url( $item['link'] ); ?>"><?php echo esc_html( $item['text'] ); ?></a>
							<?php endforeach; endif; ?>
							<?php if ( ! $nf_has_upute ) : ?>
								<a href="/hu/utmutatok/">Használati útmutatók</a>
							<?php endif; ?>
						</div>
					<?php endif; ?>
		...
		<div class="nf-wrap nf-meta">
			<div class="nf-meta-left">
				<span class="nf-region">
					<img loading="lazy" decoding="async" src="https://static.devit.software/countries/flags/rectangle/hu.svg" alt="" class="nf-flag">
					Magyarország (HU)
				</span>
				<span class="nf-copy">© <?php echo date( 'Y' ); ?> NORIKS. Minden jog fenntartva.</span>
			</div>
	...
	(function () {
		...
		form.addEventListener('submit', function (e) {
			...
			if (!email || email.indexOf('@') === -1) {
				msg.textContent = 'Adj meg egy érvényes e-mail címet.'; msg.className = 'nf-signup-msg is-error'; return;
			}
			if (!list || list === 'REPLACE_LIST_ID') {
				msg.textContent = 'A hírlevél még nincs összekötve (hiányzik a Klaviyo List ID).'; msg.className = 'nf-signup-msg is-error'; return;
			}
			msg.textContent = 'Feliratkozás…'; msg.className = 'nf-signup-msg';
			...
			fetch('https://manage.kmail-lists.com/ajax/subscriptions/subscribe', {
				...
				if (d && d.success) {
					msg.textContent = 'Köszönjük! Ellenőrizd az e-mailed a megerősítéshez.'; msg.className = 'nf-signup-msg is-ok';
					form.reset();
				} else {
					msg.textContent = (d && d.errors && d.errors[0]) ? d.errors[0] : 'Valami hiba történt, próbáld újra.';
					msg.className = 'nf-signup-msg is-error';
				}
			}).catch(function () {
				msg.textContent = 'Valami hiba történt, próbáld újra.'; msg.className = 'nf-signup-msg is-error';
			});
		});
	})();
	</script>
