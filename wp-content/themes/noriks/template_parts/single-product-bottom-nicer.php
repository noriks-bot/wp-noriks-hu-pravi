...
<div style="background: #f9f9f9; padding-top: 30px;" >
    <section style="background: #f9f9f9; max-width: 1440px;" class="comparison-section comparison-section-gray">
        <div style="background: #f9f9f9;padding: 0;padding-left: 10px; padding-right: 10px;" class="comparison-intro comparison-intro-gray ">
            ...
            Nem csak te keresed a ráncmentes, sima bőrt.
            <?php elseif ( function_exists('noriks_is_type') && noriks_is_type('bokserice') ): ?>
            
            Nem csak te keresed a tökéletes boxert.
...
// Leak boxers / kompresijske majice / kidsnest / ortopedski jastuk take precedence even if they still carry the socks category.
if ( $is_leakboxers_page || $is_kompmajice_page || $is_kidsnest_page || $is_jastuk_page ) { $is_nogavice_page = false; }

// Fallback product name shown in review cards.
$rv_fallback_title = $is_kneefix_page ? 'NORIKS KneeFix térdrögzítő'
                     : ( $is_jastuk_page ? 'NORIKS ErgoSit ortopéd ülőpárna'
                   : ( $is_kidsnest_page ? 'NORIKS KidsNest párna'
...
function get_wc_product_pool(
    ...
    if ( $product_id ) {
        ...
        if ( $is_kidsnest || $is_jastuk ) { $is_nogavice = false; }
    }
...
<?php if ( $is_nogavice_page || $is_ortopas_page || $is_bunion_page || $is_fisiorest_page || $is_norikshers_review_page || $is_leakboxers_page || $is_kompmajice_page || $is_kidsnest_page || $is_jastuk_page || $is_kneefix_page ) : ?>
<style>/* socks + belt + bunion + fisiorest + norikshers + leakboxers + kompsfit + kidsnest + jastuk: text-only reviews, no avatar */ #reviews-section .avatar { display: none !important; }</style>
<?php endif; ?>
...
if ( $is_leakboxers || $is_kompmajice || $is_kidsnest || $is_jastuk ) { $is_knc = false; } // carry sock cat but are NOT socks
...
$kneefix_faq = array(
  array(
    'questioon' => 'Alkalmas a KneeFix mindennapi viselésre?',
    'answer'    => 'Igen. A NORIKS KneeFix kifejezetten a hétköznapi mozdulatokhoz készült — járáshoz, munkához, lépcsőzéshez vagy hosszabb álláshoz.',
  ),
  array(
    'questioon' => 'Beállíthatom magam a kompressziót?',
    'answer'    => 'Igen. A beépített precíz állítókerékkel Ön állítja be a kompressziót — annyi támaszt, amennyi kellemes.',
  ),
  array(
    'questioon' => 'Lecsúszik a rögzítő járás közben?',
    'answer'    => 'A KneeFix csúszásmentes szilikonszegéllyel készült, amely segít csökkenteni a lecsúszást és a felgyűrődést viselés közben.',
  ),
  array(
    'questioon' => 'Hordhatom a rögzítőt ruha alatt?',
    'answer'    => 'Igen. A rugalmas, vékony szabás kényelmes viseletet tesz lehetővé a legtöbb hétköznapi ruha alatt.',
  ),
  array(
    'questioon' => 'Mindkét térdre jó a rögzítő?',
    'answer'    => 'Rendeléskor kiválasztja az oldalt (bal vagy jobb), így a rögzítő pontosan arra a térdre illeszkedik, amelyiket támogatni szeretné.',
  ),
  array(
    'questioon' => 'Hordhatom hosszabb ideig a rögzítőt?',
    'answer'    => 'A rögzítő mindennapi viselésre készült. Sok vásárló hordja munkában, sétán és a napi teendők közben.',
  ),
  array(
...
$kidsnest_faq = array(
  ...
  array( 'questioon' => 'Milyen korosztálynak készült a KidsNest?', 'answer' => 'A KidsNest három méretben kapható: 1–3, 3–9 és 9–14 éves korig. A legfontosabb időablak a 3. és 9. életév közé esik, amikor a szájpadlás és az állkapocs a legintenzívebben fejlődik — de minden életkornak megvan a maga mérete és a maga haszna.' ),
...
// ErgoSit ortopéd ülőpárna — termék GYIK (NORIKS).
$jastuk_faq = array(
  array( 'questioon' => 'Hogyan enyhíti a NORIKS ErgoSit az ülés közbeni fájdalmat?', 'answer' => 'Az ErgoSit farokcsont-kivágása megszünteti a farokcsontra és az ülőidegre nehezedő közvetlen nyomást, míg a nagy sűrűségű memóriahabból készült anatómiai forma egyenletesen osztja el a testsúlyt a csípőn és a combokon. Ezzel tehermentesíti az érzékeny pontokat, és támogatja az egészséges, egyenes testtartást.' ),
  array( 'questioon' => 'Hol használhatom a párnát?', 'answer' => 'Bárhol, ahol ül — az autóban, irodai széken, étkezőszéken, kerekesszékben vagy otthon. A stabil, csúszásgátló alap a helyén tartja, így a kényelem egész nap Önnel tart.' ),
  array( 'questioon' => 'Ellaposodik a hab idővel?', 'answer' => 'Nem. Az ErgoSit nagy sűrűségű memóriahabot használ, amely hosszan tartó, mindennapi használat mellett is megőrzi formáját és keménységét — ellentétben az olcsó párnákkal, amelyek gyorsan ellaposodnak.' ),
    ...
    foreach ( $list as $faq_item ): ?>
      <div class="faq-item">
        ...
        <div class="faq-answer">
          <p><?php echo function_exists('noriks_no_free_exchange') ? noriks_no_free_exchange( $faq_item["answer"] ) : $faq_item["answer"]; ?></p>
        </div>
      </div>
    <?php endforeach;
  endif;
